...
#+BEGIN_SRC emacs-lisp
  (use-package ivy-posframe
    ...
    (defun ivy-posframe-display-at-other-half-frame (str)
      (ivy-posframe--display str #'posframe-poshandler-other-half-frame))

    (dolist (command '(swiper))
      (push `(,command . ivy-posframe-display-at-other-half-frame) ivy-display-functions-alist))
    (dolist (command '(ivy-bibtex))
      (push `(,command . ivy-posframe-display-at-frame-bottom-left) ivy-display-functions-alist))
...
#+BEGIN_SRC emacs-lisp
  (use-package lsp-python-ms
    :commands lsp
    :load-path "~/Program/lsp-python-ms/"
    :hook (python-mode)
    :config
